...
生成最终博客笔记时，frontmatter 中的日期字段必须按以下语义填写：

| 字段 | 含义 | 取值来源 |
|------|------|----------|
| `pubDate` | **笔记发布/写入博客的日期** | **今天的日期**（YYYY-MM-DD），即 Agent 生成笔记的当天 |
| `published` | **原始内容的发布/上传日期** | 从源内容提取（B站视频上传日、文章发布日、PDF 日期等） |

> ⚠️ **常见错误**：不要沿用转录工具输出的 `pubDate`（那是转录时间）。最终笔记的 `pubDate` 必须是生成笔记当天。
